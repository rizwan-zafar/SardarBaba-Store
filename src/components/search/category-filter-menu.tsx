  ...
  function onClick() {
    ...
    if (Array.isArray(items) && !!items.length) {
      ...
      let currentFormState = selectedCategories.includes(slug)
        ...
        {
           let paramString:any=router.query.category;
           if(paramString.charAt(0) === "1")
          {
            router.push(
              { 
                query: {
                    category: currentFormState[1]
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className="text-brand-dark capitalize py-0.5">{name.charAt(0) === '1'?name.substring(1):name}</span>
           {depth > 0 && (
            <span
