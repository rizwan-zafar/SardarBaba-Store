...
const CategoryGridBlock: React.FC<CategoriesProps> = ({
  ...
  return (
    <div className={className}>
      <SectionHeader
        sectionHeading="What kind of Stationary You Like"
        sectionSubHeading="Here order your favorite Stationary from different categories"
        ...
        {error ? (
          ...
          data?.categories?.data?.slice(0, 16).map((category) => (
            <CategoryCard
              ...
              href={{
                pathname: ROUTES.SEARCH,
                 query: { category: category.slug.replace(/[^a-zA-Z0-9 ]/g, "")
                 .replace(/[^A-Z0-9]/gi, "-")
                 .replace(/[^A-Z0-9]/gi, " ")
                 .replace(/ {1,2}/gi, "-")
                 .toLowerCase()
